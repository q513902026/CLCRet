...
function clcret:QUEST_LOG_UPDATE()
	...
	optionFrameLoad:SetText(L["Load Options"])
...
function clcret:AuraButtonExecPlayerMissingBuff()
	...
	local name, icon, count, debuffType, duration, expirationTime, caster = LibAuraUtil.UnitBuff( data.spell,"player")
	if not name then
		button:Show()
	else
		button:Hide()
...
function clcret:AuraButtonExecGenericBuff()
	...
	if tonumber(data.spell) then data.spell = GetSpellInfo(data.spell) end 
	local name, icon, count, debuffType, duration, expirationTime, caster = LibAuraUtil.UnitBuff(data.spell,data.unit)
	if name then
...
function clcret:AuraButtonExecGenericDebuff()
	...
	if tonumber(data.spell) then data.spell = GetSpellInfo(data.spell) end 
	local name, icon, count, debuffType, duration, expirationTime, caster = LibAuraUtil.UnitDebuff(data.spell,data.unit)
	if name then
		if data.byPlayer and (caster ~= "player") then
			button:Hide()
		else
...
function clcret:COMBAT_LOG_EVENT_UNFILTERED(event)
	-- pass info for the sov function
	if db.sov.enabled then
		clcret:SOV_COMBAT_LOG_EVENT_UNFILTERED(event,CombatLogGetCurrentEventInfo())
	end
	
	-- return if no icd
	if not icd.cleu then return end
	local  timestamp, combatEvent, hideCaster, sourceGUID, sourceName, sourceFlags, sourceRaidFlags, destGUID, destName, destFlags, destRaidFlags, spellId, spellName, spellSchool, spellType, dose = CombatLogGetCurrentEventInfo()
	-- icd logic
